...
<form action="Mng_Exp_Function.php" method="post">
<?php 
session_start();
...
if(isset($_POST['writtername']))
{
   ...
   //Select data from db
   $sqlsta = "SELECT * FROM article_information where writter_name = '".$_POST['writtername']."' ";
   ...
   if(! $retvalsta ) {
	...
	echo "<tr><td>Article ID</td><td>Writer Name</td><td>Article Date</td><td>Article Title</td><td>Article Submission Date</td><td>To View Article Information</td></tr>";
	
	//Display data
	while($row = mysqli_fetch_assoc($retvalsta)) {
	
		echo "<tr><td>".$row['articleid']."</td><td>".$row['writter_name']."</td><td>".$row['date']."</td><td>".$row['title']."</td>
		<td>".$row['subdate']."</td><td><input type='submit' value = '".$row['articleid']."' name = 'reviewarticle_id'></td></tr>";
	}
...
if(isset($_POST['articledate']))
{
   ...
   //Select data from db
   $sqlsta = "SELECT * FROM article_information where date = '".$_POST['articledate']."' ";
   $retvalsta = mysqli_query($con,$sqlsta);
...
if(isset($_POST['articletitle']))
{
   ...
   //Select data from db
   $sqlsta = "SELECT * FROM article_information where title = '".$_POST['articletitle']."' ";
   $retvalsta = mysqli_query($con,$sqlsta);
